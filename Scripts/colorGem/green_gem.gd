...
func _ready():
	...
	if glow_sprite != null:
		...
		glow_sprite.modulate = Color(0.0, 1.0, 0.0, 0.8)  # 绿色发光，减少透明度
...
func _physics_process(delta):
	if is_active and player != null:
		...
		if dis <= pick_up_distance:
			# 拾取绿宝石时触发绿色调效果
			trigger_green_effect()
			Pooler.gem_collected.emit(20)  # 绿宝石价值20个普通宝石
...
func create_particles():
	...
	particles.name = "GreenGemParticles"
	...
	particle_material.color = Color(0.0, 1.0, 0.0, 1)  # 纯绿色，不透明
...
func create_bubble_particles():
	...
	color_ramp.set_color(0, Color(0.0, 1.0, 0.0, 1))  # 起始为纯绿色不透明
	color_ramp.set_color(0.5, Color(0.0, 1.0, 0.0, 0.8))  # 中间为纯绿色半透明
	color_ramp.set_color(1, Color(0.0, 1.0, 0.0, 0))  # 结束为纯绿色透明
	bubble_particles.color = Color(0.0, 1.0, 0.0, 1)  # 基础颜色为纯绿色不透明
...
func update_breathing_effect():
	...
	bubble_particles.color = Color(0.0, 1.0, 0.0, alpha)  # 纯绿色

# 触发绿色调效果
func trigger_green_effect():
	# 启动粒子效果
	if particles != null:
	...
	apply_persistent_green_effect()

	# 同时播放一次性全屏绿色效果
	play_fullscreen_green_effect()
...
# 应用持续的绿色调效果
func apply_persistent_green_effect():
	# 检查节点是否仍在场景树中
	if get_tree() == null:
		return
...
# 播放一次性全屏绿色效果
func play_fullscreen_green_effect():
	# 检查节点是否仍在场景树中
	if get_tree() == null:
		return
	...
	green_overlay.name = "GreenScreenEffect"

	# 添加到场景树
	get_tree().root.add_child(green_overlay)

	# 创建动画淡出效果
	var tween = create_tween()
	tween.tween_property(green_overlay, "color", Color(0.0, 1.0, 0.0, 0), 4.0)  # 更慢的淡出效果
	tween.tween_callback(green_overlay.queue_free)
