...
func _ready():
	...
	if glow_sprite != null:
		...
		glow_sprite.modulate = Color(1.0, 0.8, 0.0, 0.8)  # 金黄色发光，减少透明度
...
func _physics_process(delta):
	if is_active and player != null:
		...
		if dis <= pick_up_distance:
			# 拾取黄宝石时触发暖色调效果
			trigger_warm_effect()
			Pooler.gem_collected.emit(10)  # 黄宝石价值10个普通宝石
...
func update_glow_effect(delta):
	...
	if glow_sprite != null:
		...
		# 添加颜色变化效果 - 始终保持金黄色
		glow_sprite.modulate.r = 1.0
		glow_sprite.modulate.g = 0.8
		glow_sprite.modulate.b = 0.0
...
func create_particles():
	...
	particles.name = "YellowGemParticles"
	...
	particle_material.color = Color(1.0, 0.8, 0.0, 1)  # 金黄色，不透明
...
func create_bubble_particles():
	...
	color_ramp.set_color(0, Color(1.0, 0.8, 0.0, 1))  # 起始为金黄色不透明
	color_ramp.set_color(0.5, Color(1.0, 0.8, 0.0, 0.8))  # 中间为金黄色半透明
	color_ramp.set_color(1, Color(1.0, 0.8, 0.0, 0))  # 结束为金黄色透明
	bubble_particles.color = Color(1.0, 0.8, 0.0, 1)  # 基础颜色为金黄色不透明
...
func update_breathing_effect():
	...
	bubble_particles.color = Color(1.0, 0.8, 0.0, alpha)  # 金黄色

# 触发暖色调效果
func trigger_warm_effect():
	# 启动粒子效果
	if particles != null:
	...
	apply_persistent_warm_effect()

	# 同时播放一次性全屏暖色效果
	play_fullscreen_warm_effect()
...
# 应用持续的暖色调效果
func apply_persistent_warm_effect():
	# 检查节点是否仍在场景树中
	if get_tree() == null:
		return
	...
	# 创建或更新场景着色效果
	var warm_modulate = root.get_node_or_null("YellowModulate")
	if warm_modulate == null:
		warm_modulate = CanvasModulate.new()
		warm_modulate.name = "YellowModulate"
		warm_modulate.color = Color(1.029, 0.836, 0.0, 1.0)  # 暖橙色 - 红橙黄暖色调
		root.add_child(warm_modulate)
	else:
		warm_modulate.color = Color(1.111, 0.829, 0.0, 1.0)
...
# 播放一次性全屏暖色效果
func play_fullscreen_warm_effect():
	# 检查节点是否仍在场景树中
	if get_tree() == null:
		return
	...
	warm_overlay.color = Color(0.984, 0.914, 0.161, 0.922)  # 金黄色，高透明度
	warm_overlay.anchor_right = 1
	warm_overlay.anchor_bottom = 1
	warm_overlay.z_index = 100  # 确保在最上层
	warm_overlay.name = "WarmScreenEffect"

	# 添加到场景树
	get_tree().root.add_child(warm_overlay)

	# 创建动画淡出效果
	var tween = create_tween()
	tween.tween_property(warm_overlay, "color", Color(1.0, 0.8, 0.0, 0), 4.0)  # 更慢的淡出效果
	tween.tween_callback(warm_overlay.queue_free)
