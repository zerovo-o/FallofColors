...
func _ready():
	...
	if glow_sprite != null:
		...
		glow_sprite.modulate = Color(1.0, 0.0, 0.0, 0.8)  # 纯红色发光，减少透明度
...
func _physics_process(delta):
	if is_active and player != null:
		...
		if dis <= pick_up_distance:
			# 拾取红宝石时触发红色调效果
			trigger_red_effect()
			Pooler.gem_collected.emit(5)  # 红宝石价值5个普通宝石
...
func create_particles():
	...
	particles.name = "RedGemParticles"
	...
	particle_material.color = Color(1.0, 0.0, 0.0, 1)  # 纯红色，不透明
...
func create_bubble_particles():
	...
	color_ramp.set_color(0, Color(1.0, 0.0, 0.0, 1))  # 起始为纯红色不透明
	color_ramp.set_color(0.5, Color(1.0, 0.0, 0.0, 0.8))  # 中间为纯红色半透明
	color_ramp.set_color(1, Color(1.0, 0.0, 0.0, 0))  # 结束为纯红色透明
	bubble_particles.color = Color(1.0, 0.0, 0.0, 1)  # 基础颜色为纯红色不透明
...
func update_breathing_effect():
	...
	bubble_particles.color = Color(1.0, 0.0, 0.0, alpha)  # 纯红色

# 触发红色调效果
func trigger_red_effect():
	# 启动粒子效果
	if particles != null:
	...
	apply_persistent_red_effect()
	
	# 同时播放一次性全屏红色效果
	play_fullscreen_red_effect()
...
# 应用持续的红色调效果
func apply_persistent_red_effect():
	# 检查节点是否仍在场景树中
	if get_tree() == null:
		return
...
# 播放一次性全屏红色效果
func play_fullscreen_red_effect():
	# 检查节点是否仍在场景树中
	if get_tree() == null:
		return
	...
	red_overlay.name = "RedScreenEffect"
	...
	tween.tween_property(red_overlay, "color", Color(1.0, 0.0, 0.0, 0), 4.0)  # 更慢的淡出效果
	tween.tween_callback(red_overlay.queue_free)
